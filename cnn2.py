...
for i in input_file:
    ...
    dataset_dir = "./DE_CNN/3D_dataset/"+with_or_not+"_base/DE_"
    ###load training setnput_file_name,".mat")
    data_file = sio.loadmat(dataset_dir+input_file_name+".mat")

    print("loading ",dataset_dir+i
    ...
    cnn_datasets = cnn_datas = np.array(list(pd.get_dummies(labels)))
    # print("one_hot_labels:",one_hot_labels.shape)
    labels = np.asarray(pd.get_dummies(labels), dtype=np.int8)
    # shuffle data
    index = np.array(range(0, len(labels)))
    np.random.shuffle( index)ets.reshape(len(cnn_datasets)//time_step, window_size,input_height,input_width,input_channel_num)
    ...
    for curr_fold in range(0,fold):
        ...
        #                            'f1_score': test_f1})  # , 'roc_auc':test_auc})
            ...
            #file_path = "./DE_CNN/result/"+with_or_not+"/"+file_dir+"/"+arousal_or_valence+"/"+input_file_name+"_"+str(curr_fold)+".xlsx"
