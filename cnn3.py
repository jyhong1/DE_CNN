...
args = sys.argv[:]
with_or_not = args[1] #with
#input_file = args[2] # e.g. s01
#input_file = ["s01", "s02","s03","s04","s05","s06","s07","s08","s09","s10", "s11", "s12", "s13", "s14", "s15", "s16", "s17", "s18", "s19", "s20", "s21", "s22", "s23", "s24", "s25", "s26", "s27", "s28", "s29", "s30", "s31", "s32"]
input_file = ["./merged_file_test.mat"] #can ignore right now.

arousal_or_valence = args[2] # valence / arousal
inputs = list(map(int,args[3:])) # 1, 1 2, 1 2 3...
print("inputs",inputs)
...
for i in input_file:
    ...
    dataset_dir = "./merged_file_(9to16).mat"
    #dataset_dir = "./DE_CNN/3D_dataset/with_base/DE_s01.mat"
    ###load training set
    print("loading ",dataset_dir)
    data_file = sio.loadmat(dataset_dir)


    cnn_datasets = data_file["data"]
    print("shape cnn_datasets: ", cnn_datasets.shape)
    ...
    print("label_index length:", len(label_index))
    #print("label_index: ", label_index)

    labels = labels[0,[label_index]]
    labels = np.squeeze(np.transpose(labels))
    print("loaded shape:",labels.shape) #(4800, )
    lables_backup = labels
    print("cnn_dataset shape before reshape:", np.shape(cnn_datasets)) #(4800, 4, 9, 9)
    cnn_datasets = cnn_datasets.transpose(0,2,3,1) 
    print("cnn_dataset shape after reshape:", np.shape(cnn_datasets)) #(4800, 9, 9, 4)

    cnn_datasets = cnn_datasets[:,:,:,bands] #bands
    print("cnn_dataset shape 1:", np.shape(cnn_datasets))
    ...
    print("cnn_dataset shape 2:", np.shape(cnn_datasets)) #(4800, 1, 9, 9, 4)
    one_hot_labels = np.array(list(pd.get_dummies(labels)))
    print("one_hot_labels:",one_hot_labels.shape) #(2, )
    ...
    print("cnn_datasets size: ", len(cnn_datasets))
    print("labels size: ", len(labels))

    print("cnn_datasets shape: ", cnn_datasets.shape) #(4800, 1, 9, 9, 4)
    print("labels shape: ", labels.shape) #(4800, 2)
    ...
    # input placeholder
    cnn_in = tf.compat.v1.placeholder(tf.float32, shape=[None, input_height, input_width, input_channel_num], name='cnn_in')
    Y = tf.compat.v1.placeholder(tf.float32, shape=[None, n_labels], name='Y')
    ...
    y_pred = tf.argmax(input=tf.nn.softmax(y_), axis=1, name="y_pred")
    ...
    correct_prediction = tf.equal(tf.argmax(input=tf.nn.softmax(y_), axis=1), tf.argmax(input=Y, axis=1))
    ...
    ### wandb start
    #wandb.init(project='decnn', entity='wmbhoph1')
    ...
    for curr_fold in range(0,fold):
        ...
        print("indexes shape: ", indexes.shape) #(4800, )
        split_list = [i for i in range(curr_fold*fold_size,(curr_fold+1)*fold_size)]
        split = np.array(split_list)
        cnn_test_x = cnn_datasets[split]
        print("cnn_test_x shape: ", cnn_test_x.shape) #(480, 1, 9, 9, 4)

        
        for elem in cnn_test_x:
            for element in elem:
        ...
        test_y = labels[split]
        print("test_y shape: ", test_y.shape) #(480, 2)
        ...
        print("cnn_train_x shape: ", cnn_train_x.shape) #(4320, 1, 9, 9, 4)

    
        for elem in cnn_train_x:
            for element in elem:
                for elem1 in element:
                    for elem2 in elem1:
                        for elem3 in elem2:
                            if(np.isnan(elem3)):
                                print("*****There is nan value*****")
                                break 
        print("train_y shape: ", train_y.shape) # (4320, 2)
        ...
        print("batch_: ", batch_num_per_epoch)
        ...
        with tf.compat.v1.Session(config=config) as session:
            ...
            for epoch in range(training_epochs):
                ...
                if (epoch % 1 == 0):
                    ...
                    #print("train_accracy_arr: ", train_accuracy)
                    ...
                    #wandb.log({'Epoch': epoch+1, 'train_loss': np.mean(train_loss), 'train_acc':np.mean(train_accuracy)})

                    if(np.mean(train_accuracy)<0.7):
                        learning_rate=1e-5 #initial(1e-5)
                    ...
                    #wandb.log({'Epoch': epoch+1, 'test_loss': np.mean(test_loss), 'test_acc':np.mean(test_accuracy)})
        ...
        #                            'f1_score': test_f1})  # , 'roc_auc':test_auc})
            ...
            file_path = "./DE_CNN/result/"+"all"+"/"+with_or_not+"/"+"1234"+"/"+arousal_or_valence+"/"+"test(9to16)"+str(curr_fold)+".xlsx"
