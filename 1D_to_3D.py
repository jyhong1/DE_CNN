...
def pre_process(path,y_n):
	...
	# convert 128 vector ---> 4*9*9 cube (2D -> 3D Cube)
	for vector in data:
...
if __name__ == '__main__':
	dataset_dir = "./DE_CNN/1D_dataset/"
	use_baseline = sys.argv[1]
	if use_baseline=="yes":
		result_dir = "./DE_CNN/3D_dataset/with_base/"
		if os.path.isdir(result_dir)==False:
			os.makedirs(result_dir)
	else:
		result_dir = "./DE_CNN/3D_dataset/without_base/"
		if os.path.isdir(result_dir)==False:
			os.makedirs(result_dir)
